...
export function DataTableBulkActions<TData>({
  ...
  const selectedIds = useMemo(() => {
    return selectedRows.map((row) => {
      const code = row.original as SubscriptionCode
      return code.id
    })
  }, [selectedRows])
  ...
  const handleBatchEnable = async () => {
    const result = await batchUpdateSubscriptionCodeStatus(
      selectedIds,
  ...
  const handleBatchDelete = async () => {
    const result = await batchDeleteSubscriptionCodes(selectedIds)
    if (result.success) {
