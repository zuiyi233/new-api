...
const RegistrationCodeUsageModal = ({ visible, onCancel, record, t }) => {
    ...
    const loadUsages = async () => {
      ...
      try {
        const res = await API.get(
          `/api/registration-code/usage?registration_code_id=${record.id}&p=1&page_size=100`,
        );
  ...
  const exportUsage = () => {
    ...
    downloadTextAsFile(
      rows.join('\n'),
      `registration-code-usage-${record?.id || 'unknown'}.csv`,
    );
  ...
  return (
    ...
    >
      <div className='mb-3 flex flex-wrap gap-2'>
        ...
        <Tag>{record?.product_key || 'novel_product'}</Tag>
      ...
      <Table
        ...
        loading={loading}
        pagination={false}
        empty={<Empty description={t('暂无使用记录')} style={{ padding: 24 }} />}
      />
    </Modal>
  );
};

export default RegistrationCodeUsageModal;
